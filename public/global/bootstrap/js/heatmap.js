...
(function(w){
    ...
    store.prototype = {
        ...
        addDataPoint: function(x, y){
            ...
            var moheatmap = document.getElementById('moheatmap');
            if(moheatmap.dataset.moheatmap != ''){
                var tmp = JSON.parse(moheatmap.dataset.moheatmap);
                //console.log(tmp[tmp.length-1].x);
                if(tmp[tmp.length-1].x == x && tmp[tmp.length-1].y == y)
                    tmp[tmp.length-1].clicks += 1;
                else
                    tmp.push({"x":x, "y":y, "clicks":data[x][y]});
            }
                
            else{
            var tmp = [];
            tmp.push({"x":x, "y":y, "clicks":data[x][y]});}
            moheatmap.dataset.moheatmap = JSON.stringify(tmp);
